...
adonis(phyloseq::distance(Control, method="bray") ~ sample_type,data = C_metadata,perm=999)
...
adonis(phyloseq::distance(Tetra, method="bray") ~ sample_type,data = T_metadata, perm=999)
```
...
p2 <- p2 + theme(plot.title = element_text(hjust = 0.5))+scale_color_manual(values=c("#55596a", "#D45E79"))
...
p2<-p2 + theme(text = element_text(size = 19))+theme(strip.text = element_text(face="bold", size=20, color="gray40"))+ theme(axis.title.x=element_blank())+theme(axis.text.x = element_blank())
...
p2 <- p2+ theme(panel.border = element_blank())
p2<-p2 + theme(text = element_text(size = 17.5))+theme(strip.text = element_text(face="bold", size=17, color="gray40"))+ theme(axis.title.x=element_blank())+theme(axis.text.x = element_blank())
...
Observed<-p2+ geom_point(size = -1, shape = 15)+geom_boxplot(show.legend = FALSE)+guides(color = guide_legend(override.aes = list(size = 7,alpha=0.5)))+ theme(axis.title.y=element_text(size=15))
Observed
...
p2 <- p2 + theme(plot.title = element_text(hjust = 0.5))+scale_color_manual(values=c("#55596a", "#D45E79"),guide = guide_legend(override.aes = list(shape = c(rep(15, 2)),size=7,alpha=0.5)))
p2 <- p2+ theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+ theme(panel.background = element_blank())
p2 <- p2+theme(axis.line = element_line(colour = "black"))
...
adonis(phyloseq::distance(subs, method="bray") ~ treatment2,data = metadata, perm=999)
...
metadata <- as(sample_data(subs), "data.frame")
adonis(phyloseq::distance(subs, method="bray") ~ treatment3,data = metadata, perm=999)
```
...
d = phyloseq::distance(compare, "bray")
adonis(d ~ treatment3, compare.1, perm=999)

d1 = phyloseq::distance(cycle1, "bray")
adonis(d1 ~ treatment3, cycle1.2, perm=999)

d2 = phyloseq::distance(cycle2, "bray")
adonis(d2 ~ treatment3, cycle2.2, perm=999)

d3 = phyloseq::distance(cycle3_before_stress, "bray")
...
d = phyloseq::distance(cycle2_tet, "bray")
...
d = phyloseq::distance(cycle3_tet, "bray")
...
adonis(phyloseq::distance(subs1, method="bray") ~ cycle,
...
adonis(phyloseq::distance(subs4, method="bray") ~ cycle,
       data = metadata, perm=999)
```
...
metadata <- as(sample_data(subs1), "data.frame")
adonis(phyloseq::distance(subs1, method="bray") ~ treatment_cycle,data = metadata, perm=999)
...
ggsave("microbiome/output/cycle3_after_plot.png", height = 6, width = 9)
```
...
png(figure_1, 8.6 * plot_res, 9.5 * plot_res, res = plot_res)
...
ggsave("microbiome/output/nurse.png", height = 3.5, width = 6)
```
...
### compare if core taxa abundances are significantly different between controls and treatments in cycle 1, 2 and cycle 3, use Wilcoxon tests with following fdr correction. 
...
p<-ggplot(data = ps_df_sum2, aes(x = treatment3, y = Abundance, colour = treatment3)) +
  geom_point(position = position_jitter())+geom_boxplot(show.legend=FALSE, alpha = 0,colour="grey53",size=0.7)+scale_color_manual(values=c("#55596a", "#D45E79"),guide = guide_legend(override.aes = list(shape = c(rep(15, 2)),size=7,alpha=0.5)))+theme_bw()
...
ggsave("microbiome/output/ASV_Gilliamella.png", height = 4.5, width = 8.5)
```
...
neworder3 <- c("ASV0008","ASV0026","ASV0039","ASV0049")
change3 <- c("B. asteroides \nASV0008","B. asteroides \nASV0026","B. asteroides \nASV0039","B. indicum \nASV0049")
ps_df_sum4 <- arrange(transform(ps_df_sum2, ASV=factor(ASV,levels=neworder3,labels=change3)),ASV)
...
ggsave("microbiome/output/ASV_Bifidobacterium.png", height = 2.7, width = 8.5)
```
...
change3 <- c("L. apis","L. kulla-\nbergensis","L. helsing-\nborgensis","L. kunkeei","L. kimbladii","L.melliventis","not available")
ps_df_sum4 <- arrange(transform(ps_df_sum2, Species=factor(Species,levels=neworder3,labels=change3)),Species)

p<-ggplot(data = ps_df_sum4, aes(x = treatment3, y = Abundance, colour = treatment3)) +
  geom_point(position = position_jitter())+geom_boxplot(show.legend=FALSE, alpha = 0,colour="grey53",size=0.7)+scale_color_manual(values=c("#55596a", "#D45E79"),guide = guide_legend(override.aes = list(shape = c(rep(15, 2)),size=7,alpha=0.5)))
...
ggsave("microbiome/output/Lactobacillus spp.png", height = 4.50, width = 8.5)
```
...
ggsave("microbiome/output/ASV_Lactobacillus apis2.png", height = 2.5, width = 6.1)
```
...
ggsave("microbiome/output/ASV_Lactobacillus kullabergensis2.png", height = 2.5, width = 4.65)
```
...
p<-p + facet_wrap(~ASV, scales="free_y",nrow=4)+theme_bw()
p<-p + theme(strip.text = element_text(size=13,face="bold",color="grey40"))
taxa2<-p+theme(axis.title.y = element_text(size=13, face="bold"))+theme(axis.text.y = element_text(size=12, face="bold"))+theme(axis.text.x = element_blank())+theme(axis.title.x = element_blank())+theme(strip.background =element_rect(fill="white", color="white"))
taxa2<- taxa2 + theme(legend.title = element_blank(),legend.background = element_blank(),legend.key = element_blank(),legend.text = element_text(size=14, face="italic"))
taxa2+ggtitle("Lactobacillus sp. NA")+theme(plot.title = element_text(hjust = 0.5, size=14, face="bold.italic",colour = "grey38"))+theme(axis.ticks.x=element_blank())
